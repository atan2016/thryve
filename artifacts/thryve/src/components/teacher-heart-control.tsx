...
export function TeacherHeartControl({
  ...
  const nextIntent: "heart" | "unheart" = effectiveHearted ? "unheart" : "heart";

  function runToggle() {
    if (!viewerUserId || pending) return;
    setPending(true);
    try {
      enqueuePendingTeacherHeart(viewerUserId, { teacherId, teacherSlug, intent: nextIntent });
  ...
  if (variant === "compact") {
    return (
      <button
        ...
        disabled={pending}
        onClick={(e) => { e.preventDefault(); runToggle(); }}
        type="button"
      >
